...
describe('GuestbookSection', () => {
  ...
  it('shows success state after submission', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/lời chúc/i), {
      target: { value: 'Chúc mừng hạnh phúc' },
    });
    fireEvent.click(screen.getByRole('button', { name: /gửi lời chúc/i }));
    await waitFor(() => {
      expect(
        screen.getByText(/lời chúc của bạn đã được gửi/i),
      ).toBeInTheDocument();
    });
  });
