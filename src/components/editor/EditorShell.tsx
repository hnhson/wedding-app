...
export default function EditorShell({ card }: { card: Card }) {
  ...
  const save = useCallback(async (cfg: CardConfig) => {
  ...
  useEffect(() => {
    ...
    autoSaveRef.current = setTimeout(() => { setSaveStatus('unsaved'); save(config); }, 500);
    return () => clearTimeout(autoSaveRef.current);
  }, [config, save]);

  useEffect(() => {
    const h = () => { if (saveStatus === 'unsaved') navigator.sendBeacon(`/api/cards/${card.id}`, JSON.stringify({ config })); };
  ...
  return (
    <div className="flex overflow-hidden" style={{ height: 'calc(100vh - 49px)', background: '#f0f0f0' }}>

      {/* ── 1. Icon toolbar ── */}
      <div className="flex w-14 flex-shrink-0 flex-col items-center border-r border-gray-200 bg-white py-3 gap-0.5 z-10">
        {TOOLS.map(tool => (
          <button
            key={tool.id}
            onClick={() => toggleTab(tool.id)}
            title={tool.label}
            className={`group relative flex h-12 w-12 flex-col items-center justify-center rounded-xl text-[10px] transition-all gap-0.5 ${
              activeTab === tool.id
          ...
          >
            ...
            {/* Tooltip */}
            <span className="pointer-events-none absolute left-[54px] z-50 whitespace-nowrap rounded-lg bg-gray-900 px-2.5 py-1 text-xs font-medium text-white opacity-0 shadow-lg transition-opacity group-hover:opacity-100">
              {tool.label}
            </span>
          </button>
        ))}
        ...
        <Link
          href="/dashboard"
          title="Về dashboard"
          className="group relative flex h-12 w-12 flex-col items-center justify-center rounded-xl text-[10px] text-gray-400 hover:bg-gray-100 hover:text-gray-800 gap-0.5"
        >
          <ChevronLeft size={19} />
          <span>Về</span>
          <span className="pointer-events-none absolute left-[54px] z-50 whitespace-nowrap rounded-lg bg-gray-900 px-2.5 py-1 text-xs font-medium text-white opacity-0 shadow-lg transition-opacity group-hover:opacity-100">
      ...
      >
        <div className="flex h-full flex-col" style={{ width: PANEL_W }}>
          <div className="flex items-center justify-between border-b px-4 py-2.5">
            ...
            <button onClick={() => setActiveTab(null)} className="rounded-lg p-1 text-gray-400 hover:bg-gray-100">
              <ChevronLeft size={15} />
            </button>
          </div>
      ...
      >
        {/* Zoom controls */}
        <div className="absolute top-3 left-1/2 -translate-x-1/2 flex items-center gap-1 rounded-full bg-white/95 px-3 py-1.5 shadow-sm border border-gray-200 text-xs text-gray-600 backdrop-blur-sm">
          <button onClick={() => setScale(s => Math.max(0.2, +(s - 0.05).toFixed(2)))} className="h-5 w-5 flex items-center justify-center rounded hover:bg-gray-100">−</button>
          <span className="w-10 text-center font-medium">{Math.round(scale * 100)}%</span>
      ...
      <div className="flex w-60 flex-shrink-0 flex-col border-l border-gray-200 bg-white">
        ...
        <div className="flex h-10 items-center gap-2 border-b px-4">
          {saveStatus === 'saving'  && <><Save size={12} className="animate-pulse text-amber-500" /><span className="text-xs text-amber-600">Đang lưu...</span></>}
          {saveStatus === 'saved'   && <><Check size={12} className="text-green-500" /><span className="text-xs text-green-600">Đã lưu tự động</span></>}
          {saveStatus === 'unsaved' && <><Save size={12} className="text-gray-400" /><span className="text-xs text-gray-400">Chưa lưu</span></>}
        </div>
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-gray-100">
          ...
          <div className="px-4 py-3">
            <p className="mb-1.5 text-[9px] font-bold tracking-widest text-gray-400 uppercase">Thông tin</p>
            <p className="font-semibold text-gray-900 text-sm truncate">{coupleName}</p>
            {config.weddingDate && (
              <p className="mt-0.5 text-xs text-gray-400">
                {new Date(config.weddingDate).toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit', year: 'numeric' })}
              </p>
            )}
          </div>
          ...
          <div className="px-4 py-3">
            <p className="mb-2 text-[9px] font-bold tracking-widest text-gray-400 uppercase">Bản xem trước</p>
            <div
              className="relative overflow-hidden rounded-xl border border-gray-100 bg-gray-50 cursor-pointer hover:border-gray-300 transition-colors"
              style={{ height: 164 }}
            ...
            >
              ...
              <div className="absolute inset-0 flex items-end justify-center pb-2 opacity-0 hover:opacity-100 transition-opacity bg-black/10">
                <span className="rounded-full bg-white px-2 py-0.5 text-[10px] font-medium text-gray-700 shadow">Xem đầy đủ</span>
              </div>
            </div>
          </div>
          ...
          {/* Share link */}
          <div className="px-4 py-3">
            <p className="mb-2 text-[9px] font-bold tracking-widest text-gray-400 uppercase">Link chia sẻ</p>
            <div className="flex items-center gap-1 rounded-lg border border-gray-200 bg-gray-50 px-2 py-1.5">
              <span className="flex-1 truncate text-[11px] text-gray-500">/invitation/{slug}</span>
              <button onClick={copyLink} className="flex-shrink-0 rounded p-1 hover:bg-gray-200 transition-colors" title="Sao chép">
          ...
          {/* Actions */}
          <div className="px-4 py-3">
            <p className="mb-2 text-[9px] font-bold tracking-widest text-gray-400 uppercase">Thao tác nhanh</p>
            <div className="space-y-1.5">
              <a
                href={`/invitation/${slug}`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2 rounded-lg border border-gray-100 px-3 py-2 text-xs text-gray-600 hover:bg-gray-50 hover:border-gray-200 transition-all"
              >
        ...
        <div className="border-t p-3">
          <button
            ...
            className="flex w-full items-center justify-center gap-1.5 rounded-xl bg-gray-900 py-2.5 text-xs font-medium text-white hover:bg-gray-700 transition-colors"
          >
