...
export default function MapPanel({ config, onChange }: Props) {
  ...
  const [error, setError] = useState<string>(() =>
    process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY
      ? ''
      : 'Chưa cấu hình Google Maps API key (NEXT_PUBLIC_GOOGLE_MAPS_API_KEY)',
  );
    ...
    autocomplete.addListener('place_changed', () => {
      ...
      onChange({
        venue: {
          ...
          mapUrl:
            place.url ??
            `https://maps.google.com/?q=${place.geometry.location.lat()},${place.geometry.location.lng()}`,
        },
      });
    });
  }, [loaded, onChange]);
      ...
      <div className="space-y-1">
        ...
        {error ? (
          ...
          <Input
            ref={inputRef}
            placeholder={
              loaded ? 'Nhập tên địa điểm...' : 'Đang tải Google Maps...'
            }
            disabled={!loaded}
          />
        )}
